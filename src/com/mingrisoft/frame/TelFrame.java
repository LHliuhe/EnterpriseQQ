...
public class TelFrame extends JFrame {
     ...
     *            - 用户树
     */
    private TelFrame(DatagramSocket ssArg, DatagramPacket dpArg, final ChatTree treeArg) {
        ...
        try {
            ...
            receiveText.addComponentListener(new ComponentAdapter() {
                public void componentResized(final ComponentEvent e) {
                    scrollPane.getVerticalScrollBar().setValue(receiveText.getHeight());
                }
            });
            ...
            // 标签使用HTML超文本格式显示对方信息
            otherSideInfo.setText("<html><body background='" + imgPath + "'><table width='" + rightPanelWidth
                    + "'><tr><td>用户名：<br>&nbsp;&nbsp;" + user.getName() + "</td></tr><tr><td>主机名：<br>&nbsp;&nbsp;"
                    + user.getHost() + "</td></tr>" + "<tr><td>IP地址：<br>&nbsp;&nbsp;" + user.getIp()
                    + "</td></tr><tr><td colspan='2' height=" + this.getHeight() * 2
                    + "></td></tr></table></body></html>");

            panel_3.add(label_1, BorderLayout.NORTH);
            label_1.setIcon(new ImageIcon(EQ.class.getResource("/image/telFrameImage/telUserImage.png")));
        } catch (Exception e) {
            e.printStackTrace();
        }
        setVisible(true);
    ...
    class sendActionListener implements ActionListener {
        public void actionPerformed(final ActionEvent e) {
            ...
            try {
                ...
                tdp = new DatagramPacket(tmpBuf, tmpBuf.length, new InetSocketAddress(ip, 1111));
        ...
        Thread t = new Thread() {// 创建匿名线程内部类
            public void run() {// 重写run方法
                ...
                try {
                    ...
                    DatagramPacket tdp = new DatagramPacket(tmpBuf, tmpBuf.length, new InetSocketAddress(ip, 1111));
                    ss.send(tdp);// UDP套接字发送数据包;
                } catch (IOException e1) {
                    e1.printStackTrace();
                } finally {
